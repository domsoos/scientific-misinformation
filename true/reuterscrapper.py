...
article_count = 0
title_list = []
while len(list_articles_url) < 3200:

    URL = 'https://www.reuters.com/news/archive/technologyNews?view=page&page='+str(count)+'&pageSize=10'
    count += 1
    ...
    for article in articles:
        ...
        # handle duplication
        newtag = a_tag.attrs['href'].split('-')
        # id number is the last element in url
        newtag.pop(-1)
        newtag = "-".join(str(x) for x in newtag)
        #print(newtag)

        if newtag not in title_list:
            title_list.append(newtag)
            list_articles_url.append(a_tag.attrs['href'])
            article_count += 1
...
for item in list_articles_url:
    try:
        url = 'https://www.reuters.com/news' + item
        #print(url)
